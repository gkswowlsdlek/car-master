...
export function writeCollection<T>(key: string, value: T[]) {
  ...
  localStorage.setItem(
    key,
    JSON.stringify({ version: STORAGE_VERSION, items: value } satisfies VersionedCollection<T>),
  );
...
export function subscribeToStorage(key: string, listener: () => void) {
  ...
  window.addEventListener(`car-master:${key}`, customListener);
  return () => {
    window.removeEventListener("storage", storageListener);
    window.removeEventListener(`car-master:${key}`, customListener);
  };
}
