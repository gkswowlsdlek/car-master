...
type InstallerApplicationJoinRow = {
  user_id: string;
  shop_name: string;
  ...
  installer_approvals:
    | { status: InstallerApprovalStatus; review_note: string | null }
    | { status: InstallerApprovalStatus; review_note: string | null }[];
};
...
export type AdminMember = {
  ...
  approvalStatus: InstallerApprovalStatus | null;
};
type DealerProfileJoinRow = {
  user_id: string;
  name: string;
  phone: string;
...
export class SupabaseMembershipRepository {
  ...
  async getInstallerApplications(): Promise<InstallerApplication[]> {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "user_id,shop_name,representative_name,business_name,business_registration_number,address,detail_address,phone,contact_phone,supported_services,supported_brands,profiles!inner(email,created_at),installer_approvals!inner(status,review_note)",
      )
      .limit(300);
    ...
    return ((data ?? []) as unknown as InstallerApplicationJoinRow[]).map((row) => {
      ...
      return {
        ...
        reviewNote: approval?.review_note ?? undefined,
        createdAt: profile?.created_at,
      } as InstallerApplication;
    });
  }
  ...
  async getAllMembers(): Promise<AdminMember[]> {
    ...
    const [dealerResult, installerApplications] = await Promise.all([
      supabase
        ...
        .select("user_id,name,phone,company_name,profiles!inner(email,created_at)")
        .limit(300),
