...
export class SupabaseProfileRepository {
  async getById(userId: string, role: "dealer" | "shop"): Promise<UserProfile | null> {
    ...
    return data ? { id: userId, role, name: data.shop_name, representativeName: data.representative_name, address: `${data.address}${data.detail_address ? ` ${data.detail_address}` : ""}`, phone: data.phone, email, brands: data.supported_brands, works: data.supported_services, hours: data.business_hours ?? undefined, closedDays: data.closed_days ?? undefined, introduction: data.introduction ?? undefined, emergencyAvailable: data.emergency_available, notifications: { request: true, chat: true, schedule: true, marketing: false } } : null;
  }
