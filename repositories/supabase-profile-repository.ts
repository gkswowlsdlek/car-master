  ...
  async getById(userId: string, role: "dealer" | "shop"): Promise<UserProfile | null> {
    ...
    if (role === "dealer") {
      const { data: dealerData } = await supabase
        ...
        .select("name,phone,company_name,activity_region")
        .eq("user_id", userId)
        .single();
      const data = dealerData as {
        name: string;
        phone: string;
      ...
      return data
        ? {
            ...
            activityArea: data.activity_region ?? undefined,
    ...
    const { data: installerData } = await supabase
      ...
      .select(
        "shop_name,representative_name,address,detail_address,phone,contact_phone,supported_brands,supported_services,business_hours,closed_days,introduction,emergency_available",
      )
      .eq("user_id", userId)
      .single();
    const data = installerData as {
      shop_name: string;
    ...
    return data
      ? {
          ...
          detailAddress: data.detail_address ?? undefined,
          phone: data.phone,
          email,
          brands: data.supported_brands,
          works: data.supported_services,
          hours: data.business_hours ?? undefined,
    ...
    const values =
      profile.role === "dealer"
        ? {
            ...
            activity_region: profile.activityArea ?? null,
        ...
        : {
            ...
            detail_address: profile.detailAddress ?? null,
            phone: profile.phone,
            supported_brands: profile.brands ?? [],
            supported_services: profile.works ?? [],
            business_hours: profile.hours ?? null,
            closed_days: profile.closedDays ?? null,
            introduction: profile.introduction ?? null,
            emergency_available: Boolean(profile.emergencyAvailable),
            updated_at: new Date().toISOString(),
          };
