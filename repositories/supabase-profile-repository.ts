...
export class SupabaseProfileRepository {
  async getById(userId: string, role: "dealer" | "shop"): Promise<UserProfile | null> {
    ...
    if (!base) return null;
    const email = authData.user?.id === userId ? authData.user.email ?? base.email : base.email;
    if (role === "dealer") {
      ...
      return data ? { id: userId, role, name: data.name, phone: data.phone, email, companyName: data.company_name ?? undefined, activityArea: data.activity_region ?? undefined, notifications: { request: true, chat: true, schedule: true, marketing: false } } : null;
    }
    ...
    return data ? { id: userId, role, name: data.shop_name, representativeName: data.representative_name, address: data.address, detailAddress: data.detail_address ?? undefined, phone: data.phone, email, brands: data.supported_brands, works: data.supported_services, hours: data.business_hours ?? undefined, closedDays: data.closed_days ?? undefined, introduction: data.introduction ?? undefined, emergencyAvailable: data.emergency_available, notifications: { request: true, chat: true, schedule: true, marketing: false } } : null;
  }
