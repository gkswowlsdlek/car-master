...
export class LocalProfileRepository implements ProfileRepository {
  getAll = () =>
    readCollection<UserProfile>(PROFILE_STORAGE_KEY).map((profile) => ({
      ...profile,
  ...
  create(profile: UserProfile) {
    if (this.getById(profile.id)) throw new Error(`Profile ${profile.id} already exists.`);
    writeCollection(PROFILE_STORAGE_KEY, [...this.getAll(), profile]);
  }
  update(profile: UserProfile) {
    if (!this.getById(profile.id)) throw new Error(`Profile ${profile.id} was not found.`);
    writeCollection(
      PROFILE_STORAGE_KEY,
      this.getAll().map((item) => (item.id === profile.id ? profile : item)),
    );
  }
  save(profile: UserProfile) {
    if (this.getById(profile.id)) this.update(profile);
    else this.create(profile);
  }
  subscribe(listener: () => void) {
    return subscribeToStorage(PROFILE_STORAGE_KEY, listener);
  }
}
