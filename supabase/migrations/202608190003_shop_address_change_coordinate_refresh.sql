-- update_shop_operating_profile (202608140001) lets an approved Shop's
-- owner/manager change `address` freely, but never touched
-- latitude/longitude at all. Those columns are only ever written today by
-- admin_register_shop (202608190002) at creation time — so after a Shop
-- edits its own address, latitude/longitude keep pointing at the OLD
-- address forever. installer-search.ts / InstallerDirectoryScreen.tsx
-- happily compute a real-looking distance from that stale coordinate; a
-- silently wrong "3.2km" is worse than the existing, already-supported
-- "거리 정보 없음" state for a NULL-coordinate Shop.
--
-- This migration only widens update_shop_operating_profile:
--   - When the payload includes latitude/longitude, store them (the caller
--     — ShopManagementScreen.tsx — geocoded the new address client-side via
--     the same services/geocoding-service.ts used by Admin registration,
--     before calling this RPC).
--   - When `address` changed but the payload did NOT include fresh
--     coordinates (geocoding failed, was rate-limited, or NAVER
--     credentials aren't configured), latitude/longitude are cleared to
--     NULL rather than left pointing at the old address. No fallback
--     coordinate is ever fabricated here, matching the admin_register_shop
--     precedent.
--   - When `address` did not change, latitude/longitude are left exactly
--     as they were (unless the payload explicitly includes them).
--
-- No new columns, no RLS change, no change to the approval/membership
-- authorization checks below — copied verbatim from 202608140001.
--
...
begin
  ...
  next_address := case when p_payload ? 'address'
    ...
    else current_shop.address end;
  address_changed := next_address is distinct from current_shop.address;

  next_latitude := case
    when p_payload ? 'latitude' then nullif(p_payload ->> 'latitude', '')::double precision
    when address_changed then null
    else current_shop.latitude
  end;
  next_longitude := case
    when p_payload ? 'longitude' then nullif(p_payload ->> 'longitude', '')::double precision
    when address_changed then null
    else current_shop.longitude
  end;
...
comment on function public.update_shop_operating_profile(uuid, jsonb) is
  'Updates only approved Shop operating fields for an active owner or manager membership. Clears latitude/longitude to NULL when address changes without a fresh geocoded coordinate, so a stale coordinate is never kept.';
