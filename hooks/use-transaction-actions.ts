...
export function useTransactionActions({
  ...
  const notifyNewMessage = useCallback((transaction: Transaction, message: TransactionChatMessage) => {
    ...
    void notificationService.notify({
      ...
      roomId: transaction.chatRoomId,
      recipientId,
      preview: message.text || "사진을 보냈습니다",
    });
  }, []);

  const sendMessage = useCallback(
    async (transaction: Transaction, message: TransactionChatMessage) => {
      if (useSupabaseData) {
        try {
          ...
          throw new Error(error instanceof Error ? error.message : "메시지를 전송하지 못했습니다.");
        }
        return;
      }
      ...
      transactionRepository.update({
        ...transaction,
        lastMessage: message.text || (message.attachments?.length ? "사진을 보냈습니다" : transaction.lastMessage),
        status: { ...transaction.status, updatedAt: message.createdAt },
      });
      notifyNewMessage(transaction, nextMessage);
    },
  ...
  const markRoomRead = useCallback(
    async (roomId: string) => {
      try {
        ...
        /* read-receipts are best-effort and must never block the chat UI */
      }
    },
  ...
  const hideTransaction = useCallback(
    async (id: string, targetRole: "dealer" | "shop") => {
      if (useSupabaseData) {
        try {
          await supabaseTransactionRepository.setVisibility(id, true);
          await refresh();
        } catch (error) {
          alert(translateTransactionError(error, "거래를 숨길 수 없습니다."));
        }
        return;
      }
      ...
      if (target && isSharedDemoTransaction(target)) {
        try {
          await demoTransactionRepository.setVisibility(id, true, targetRole);
          await refresh();
        } catch (error) {
          alert(translateTransactionError(error, "거래를 숨길 수 없습니다."));
        }
  ...
  const unhideTransaction = useCallback(
    async (id: string, targetRole: "dealer" | "shop") => {
      if (useSupabaseData) {
        try {
          await supabaseTransactionRepository.setVisibility(id, false);
          await refresh();
        } catch (error) {
          alert(translateTransactionError(error, "숨김을 해제할 수 없습니다."));
        }
        return;
      }
      const target = transactions.find((item) => item.id === id);
      if (target && isSharedDemoTransaction(target)) {
        try {
          await demoTransactionRepository.setVisibility(id, false, targetRole);
          await refresh();
        } catch (error) {
          alert(translateTransactionError(error, "숨김을 해제할 수 없습니다."));
        }
      ...
      if (targetRole === "dealer") transactionRepository.unhideForDealer(id);
      else transactionRepository.unhideForInstaller(id);
    },
    [isSharedDemoTransaction, refresh, transactions, useSupabaseData],
  );

  const changeStage = useCallback(
    async (transaction: Transaction, stage: TransactionStage) => {
      const next = transitionStage(transaction, stage, role === "shop" ? "shop" : "dealer");
      if (useSupabaseData) {
        await supabaseTransactionRepository.transitionStage(transaction.id, stage);
        await refresh();
      } else if (isSharedDemoTransaction(transaction)) {
        await demoTransactionRepository.transitionStage(transaction.id, stage, demoActorRole);
        await refresh();
      ...
      if (stage === "시공예약")
        void notificationService.notify({
          ...
          transactionId: transaction.id,
          stage,
          recipientId: transaction.dealerId,
        });
    },
  ...
  const endOutcome = useCallback(
    async (transaction: Transaction, outcome: "취소" | "시공불가", note?: string) => {
      if (useSupabaseData) {
        await supabaseTransactionRepository.endOutcome(transaction.id, outcome, note);
        await refresh();
        return;
      }
      if (isSharedDemoTransaction(transaction)) {
  ...
  const setContactStatus = useCallback(
    async (transaction: Transaction, status: ContactStatus) => {
      try {
        if (useSupabaseData) {
          await supabaseTransactionRepository.setContactStatus(transaction.id, status);
          await refresh();
          return;
        }
        transactionRepository.update({
        ...
        alert(translateTransactionError(error, "연락 결과를 저장하지 못했습니다."));
      }
    },
  ...
  const changeFinalPrice = useCallback(
    async (transaction: Transaction, finalPrice: number) => {
      try {
        if (useSupabaseData) {
          await supabaseTransactionRepository.setFinalPrice(transaction.id, finalPrice);
          await refresh();
        } else if (isSharedDemoTransaction(transaction)) {
          await demoTransactionRepository.setFinalPrice(transaction.id, finalPrice, demoActorRole);
          await refresh();
        } else
          transactionRepository.update({
            ...transaction,
            pricing: { ...transaction.pricing, finalPrice },
            status: { ...transaction.status, updatedAt: new Date().toISOString() },
          });
      } catch (error) {
        alert(translateTransactionError(error, "최종 금액을 저장할 수 없습니다."));
      }
    },
  ...
  const changePayment = useCallback(
    async (transaction: Transaction, status: PaymentStatus) => {
      try {
        const next = transitionPayment(transaction, status, role === "admin" ? "admin" : role);
        if (useSupabaseData) {
          await supabaseTransactionRepository.transitionPayment(transaction.id, status);
          await refresh();
        } else if (isSharedDemoTransaction(transaction)) {
          await demoTransactionRepository.transitionPayment(transaction.id, status, demoActorRole);
          await refresh();
        } else transactionRepository.update(next);
      } catch (error) {
        alert(translateTransactionError(error, "결제 상태를 변경할 수 없습니다."));
      }
    },
    [demoActorRole, isSharedDemoTransaction, refresh, role, useSupabaseData],
  );
