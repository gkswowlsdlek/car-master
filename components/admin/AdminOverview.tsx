...
/** Mon-Sun counts of this calendar week's transactions, by status.createdAt. */
function weeklyVolume(transactions: Transaction[]) {
  const now = new Date();
  const monday = new Date(now.getFullYear(), now.getMonth(), now.getDate() + (now.getDay() === 0 ? -6 : 1 - now.getDay()));
  return WEEKDAY_LABELS.map((_, index) => {
    ...
    return transactions.filter((item) => { const created = new Date(item.status.createdAt); return created >= dayStart && created < dayEnd; }).length;
  });
}
...
export function AdminOverview({ transactions, rooms, demoSession = false }: { transactions: Transaction[]; rooms: ChatRoom[]; demoSession?: boolean }) {
  ...
  const stalled = transactions.filter((item) => !["작업완료", "출고", "취소", "시공불가"].includes(item.status.stage) && referenceTime - new Date(item.status.updatedAt).getTime() > 1000 * 60 * 60 * 24 * 3).length;
  ...
  const stats = [
    ...
    { label: "진행 중", value: transactions.filter((item) => !["작업완료", "출고", "취소", "시공불가"].includes(item.status.stage)).length, note: "현재 운영 거래", icon: Activity, tone: "violet" },
