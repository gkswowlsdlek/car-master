...
export function InstallerApprovalPanel({ demoSession = false }: { demoSession?: boolean }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    void demoMembershipRepository.isSchemaReady().then((ready) => {
      if (active) setDemoSchemaReady(ready);
    });
    return () => {
      active = false;
    };
  ...
  const [items, setItems] = useState<InstallerApplication[]>([]);
  const [selected, setSelected] = useState<InstallerApplication | null>(null);
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(membershipEnabled);
  const load = useCallback(async () => {
    if (!membershipEnabled) return;
    setLoading(true);
    try {
      const applications = useDemoBackend
        ? await demoMembershipRepository.getInstallerApplications()
        : await membershipRepository.getInstallerApplications();
  ...
  if (!membershipEnabled)
    return (
      <section id="installer-approval-panel" className="card installer-approval-panel">
        <header>
          <div>
            <p className="eyebrow">INSTALLER MEMBERSHIP</p>
            <h2>시공점 가입 승인</h2>
          </div>
        </header>
        <div className="compact-empty">
          <b>
            {demoSession
              ? demoSchemaReady === null
                ? "시험 관리자 화면을 확인하는 중입니다."
                : "시험 관리자 화면입니다."
              : "DB 연결 후 사용할 수 있습니다."}
          </b>
          <span>
            {demoSession
              ? demoSchemaReady === null
                ? "잠시만 기다려 주세요."
                : "실제 서비스에서는 회원 관리자 계정으로 로그인하면 가입 심사 화면이 표시됩니다."
              : "Supabase 환경변수와 마이그레이션을 적용하면 가입 신청이 표시됩니다."}
          </span>
        </div>
      </section>
    );
  return (
    <section id="installer-approval-panel" className="card installer-approval-panel">
      ...
      {loading ? (
        <div className="compact-empty">
          <b>가입 신청을 불러오는 중입니다.</b>
        </div>
      ) : items.length === 0 ? (
        <div className="compact-empty">
          <b>접수된 시공점 가입 신청이 없습니다.</b>
        </div>
      ) : (
        <div className="installer-approval-layout">
          <div className="installer-application-list">
            {items.map((item) => (
              <button
                key={item.userId}
                className={selected?.userId === item.userId ? "selected" : ""}
              ...
              >
                ...
                <span>
                  <b>{item.shopName}</b>
                  <small>
                    {item.businessRegistrationNumber} · {item.representativeName}
                  </small>
                </span>
                <em className={`approval-${item.status}`}>{INSTALLER_APPROVAL_STATUS_LABEL[item.status]}</em>
              </button>
            ))}
          </div>
          {selected && (
            <aside>
              <header>
                ...
                <em className={`approval-${selected.status}`}>{INSTALLER_APPROVAL_STATUS_LABEL[selected.status]}</em>
              </header>
              <dl>
                <div>
                  <dt>사업자명</dt>
                ...
                <div>
                  <dt>연락처</dt>
                  <dd>
                    {selected.phone} · {selected.contactPhone}
                  </dd>
                </div>
                <div>
                  <dt>가능 작업</dt>
                  <dd>{selected.supportedServices.join(", ")}</dd>
                </div>
                <div>
                  <dt>취급 브랜드</dt>
                  <dd>{selected.supportedBrands.join(", ")}</dd>
                </div>
              </dl>
              <div className="approval-actions">
                <button className="approve" onClick={() => void review("approved")}>
                  <CheckCircle2 size={16} /> 승인
                </button>
                <button className="reject" onClick={() => void review("rejected")}>
                  <XCircle size={16} /> 반려
                </button>
                <button className="suspend" onClick={() => void review("suspended")}>
                  <PauseCircle size={16} /> 활동 정지
                </button>
                {selected.status !== "pending" && (
                  <button onClick={() => void review("pending")}>심사 대기로 전환</button>
                )}
              </div>
            </aside>
          )}
