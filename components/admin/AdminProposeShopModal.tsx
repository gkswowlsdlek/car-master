...
export function AdminProposeShopModal({
  requestId,
  registeredShopId,
  onClose,
  onProposed,
  ...
  return (
    <div className="stage-confirm-overlay" role="dialog" aria-modal="true" aria-label="Dealer에게 시공점 제안">
      ...
      <div className="stage-confirm-card admin-quick-shop-card">
        ...
        {registeredShopId && (
          <button
            type="button"
            className="button button-primary admin-propose-quick-pick"
            onClick={() => void propose(registeredShopId)}
            disabled={submittingId === registeredShopId}
          >
        ...
        {results &&
          (results.length === 0 ? (
            <p className="compact-empty-note">검색 결과가 없습니다.</p>
          ) : (
            <ul className="admin-propose-results">
              {results.map((shop) => (
                <li key={shop.id}>
                  <div>
                    <b>{shop.shopName}</b>
                    ...
                    {shop.supportedServices.length > 0 && <em>{shop.supportedServices.join(", ")}</em>}
                  </div>
                  <button
                    type="button"
                    className="button button-secondary"
                    onClick={() => void propose(shop.id)}
                    disabled={submittingId === shop.id}
                  >
                    {submittingId === shop.id ? "제안 중…" : "이 시공점 제안"}
                  </button>
                </li>
              ))}
            </ul>
