...
export function AdminShopSearchRequestPanel({ demoSession = false }: { demoSession?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSelectedId((current) =>
        current && requests.some((item) => item.id === current) ? current : (requests[0]?.id ?? null),
      );
      setError("");
    } catch {
  ...
  const startProcessing = async (id: string) => {
    setActionError("");
    try {
      await shopSearchRequestRepository.startProcessing(id);
      await load();
    } catch {
      setActionError("확인 시작 처리를 하지 못했습니다.");
    }
  };
  const markUnable = async (id: string) => {
    ...
    setActionError("");
    try {
      await shopSearchRequestRepository.markUnableToConnect(id);
      await load();
    } catch {
  ...
  const saveNote = async () => {
    ...
    setActionError("");
    try {
      await shopSearchRequestRepository.setAdminNote(selected.id, noteDraft);
      await load();
    } catch {
      setActionError("운영 메모를 저장하지 못했습니다.");
    }
  };

  return (
    <section id="admin-shop-search-request-panel" className="card admin-shop-search-request-panel">
      <header>
        <div>
          <p className="eyebrow">SHOP SEARCH REQUESTS</p>
          <h2>시공점 찾기 요청</h2>
        </div>
            ...
            {items.map((item) => (
              ...
              >
                ...
                <span>
                  ...
                  <small>
                    {item.dealerName} · {item.region}
                  </small>
                </span>
                <em className={`shop-search-request-status-${item.status}`}>
                  {SHOP_SEARCH_REQUEST_STATUS_LABEL[item.status]}
                </em>
              </button>
            ))}
          </div>
            ...
            <aside>
              <header>
                <div>
                  ...
                  <p>{selected.dealerPhone}</p>
                </div>
                <em className={`shop-search-request-status-${selected.status}`}>
                  {SHOP_SEARCH_REQUEST_STATUS_LABEL[selected.status]}
                </em>
              </header>
              <dl>
                <div>
                ...
                <div>
                  <dt>희망 입고일</dt>
                  <dd>{dateLabel(selected.desiredInboundDate)}</dd>
                </div>
                <div>
                  <dt>날짜 조정 가능 여부</dt>
                  <dd>{selected.dateFlexible ? "가능" : "불가"}</dd>
                </div>
                <div>
                  <dt>딜러 메모</dt>
                  <dd>{selected.dealerNote || "작성된 메모가 없습니다."}</dd>
                </div>
                <div>
                  <dt>요청 일시</dt>
              ...
              <div className="approval-actions">
                {selected.status === "requested" && (
                  <button className="approve" onClick={() => void startProcessing(selected.id)}>
                    <PlayCircle size={16} /> 확인 시작
                  </button>
                )}
                {(selected.status === "requested" || selected.status === "in_progress") && (
                  <button onClick={() => setProposalOpen(true)}>
                    <Send size={16} /> Dealer에게 제안
                  </button>
                )}
                {(selected.status === "requested" ||
                  selected.status === "in_progress" ||
                  selected.status === "shop_proposed") && (
                  <button className="reject" onClick={() => void markUnable(selected.id)}>
                    <XCircle size={16} /> 연결 어려움
                  </button>
                )}
                {!selected.registeredShopId && (
                  <button onClick={() => setRegistrationOpen(true)}>
                    <Store size={16} /> 시공점 빠른 등록
                  </button>
                )}
              </div>
              {selected.registeredShopId && (
                <p className="admin-quick-shop-registered-note">
                  <Store size={14} /> 이 요청으로 시공점이 등록되었습니다.
                </p>
              )}
              {selected.status === "shop_proposed" && selected.proposedShopName && (
                <p className="admin-quick-shop-registered-note">
                  <Send size={14} /> 제안 중: {selected.proposedShopName} (Dealer 응답 대기)
                </p>
              )}
              <div className="admin-note-field">
                <label htmlFor="shop-search-request-admin-note">운영 메모 (Dealer에게 노출되지 않음)</label>
                <textarea
                  id="shop-search-request-admin-note"
                  rows={3}
                  value={noteDraft}
                  onChange={(event) => setNoteDraft(event.target.value)}
                  placeholder="예: 미사 A업체 전화 안 받음"
                />
                <button className="button button-secondary" onClick={() => void saveNote()}>
                  메모 저장
                </button>
              </div>
            </aside>
      ...
      {demoSession && (
        <p className="compact-empty-note">
          Demo 세션에서는 실제 Supabase 계정으로 로그인해야 이 패널이 실제 데이터를 표시합니다.
        </p>
      )}
      {registrationOpen && selected && (
        <AdminQuickShopRegistrationModal
      ...
      {proposalOpen && selected && (
        <AdminProposeShopModal
          requestId={selected.id}
          registeredShopId={selected.registeredShopId}
