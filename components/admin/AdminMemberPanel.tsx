...
export function AdminMemberPanel({ demoSession = false }: { demoSession?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const rows = demoSession
        ? await demoMembershipRepository.getAllMembers()
        : await membershipRepository.getAllMembers();
      setMembers(rows);
      setError("");
    } catch {
  ...
  return (
    <section className="card admin-member-panel">
      <header>
        <div>
          <p className="eyebrow">MEMBER DIRECTORY</p>
          <h2>회원 관리</h2>
          <p>가입한 Dealer와 Installer를 한눈에 확인합니다.</p>
        </div>
      </header>
      <div className="transaction-summary-strip admin-member-summary">
        <button type="button" className={role === "전체" ? "active" : ""} onClick={() => setRole("전체")}>
          <span>전체 회원</span>
          <b>{members.length}</b>
        </button>
        <button type="button" className={role === "dealer" ? "active" : ""} onClick={() => setRole("dealer")}>
          <span>Dealer</span>
          <b>{dealerCount}</b>
        </button>
        <button type="button" className={role === "installer" ? "active" : ""} onClick={() => setRole("installer")}>
          <span>Installer</span>
          <b>{installerCount}</b>
        </button>
        <button
          type="button"
          className={approval === "pending" ? "active" : ""}
          onClick={() => {
            setRole("installer");
            setApproval("pending");
          }}
      ...
      <div className="admin-member-tools">
        ...
        <select value={approval} onChange={(event) => setApproval(event.target.value as ApprovalFilter)}>
          <option value="전체">전체 승인상태</option>
      ...
      {loading ? (
        <div className="compact-empty">
          <b>회원 목록을 불러오는 중입니다.</b>
        </div>
      ) : visible.length === 0 ? (
        <div className="compact-empty">
          <b>조건에 맞는 회원이 없습니다.</b>
          <span>검색어 또는 필터를 변경해 주세요.</span>
        </div>
      ) : (
          ...
          <div className="admin-member-list">
            {visible.map((item) => (
              <button
                ...
                onClick={() => setSelectedId((current) => (current === item.userId ? "" : item.userId))}
              >
                {item.role === "dealer" ? <Users size={18} /> : <Building2 size={18} />}
                <span>
                  <b>{item.companyName}</b>
                  <small>
                    {item.name} · {item.email || "이메일 없음"}
                  </small>
                </span>
                <span className="admin-member-badges">
                  <em className={`member-role-badge role-${item.role}`}>{roleLabel[item.role]}</em>
                  {item.approvalStatus && (
                    <em className={`approval-${item.approvalStatus}`}>{approvalLabel[item.approvalStatus]}</em>
                  )}
                </span>
              </button>
            ))}
          ...
          {selected && (
            <aside>
              <header>
                ...
                <em className={`member-role-badge role-${selected.role}`}>{roleLabel[selected.role]}</em>
              </header>
              <dl>
                <div>
                  <dt>이메일</dt>
                ...
                <div>
                  <dt>가입일</dt>
                  <dd>{selected.createdAt ? new Date(selected.createdAt).toLocaleDateString("ko-KR") : "-"}</dd>
                </div>
                {selected.approvalStatus && (
                  <div>
                    <dt>승인 상태</dt>
                    <dd>
                      <em className={`approval-${selected.approvalStatus}`}>
                        {approvalLabel[selected.approvalStatus]}
                      </em>
                    </dd>
                  </div>
                )}
              </dl>
              {selected.role === "installer" && selected.approvalStatus !== "approved" && (
                <p className="admin-member-hint">
                  승인/거절 처리는 아래 <a href="#installer-approval-panel">시공점 가입 승인</a> 패널에서 진행할 수
                  있습니다.
                </p>
              )}
            </aside>
