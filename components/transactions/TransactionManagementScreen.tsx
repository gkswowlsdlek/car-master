...
import { TransactionChatWorkspace } from "./TransactionChatWorkspace";

const stages: TransactionStage[] = ["접수", "입고예정", "입고", "시공중", "완료"];
const won = (value?: number) => value == null ? "미확정" : `${value.toLocaleString("ko-KR")}원`;

export function TransactionManagementScreen({ role, userId, transactions, rooms, selectedId, useRemoteAttachments, onSelect, onSend, onHide, onFinalPriceChange, onStageChange, onPaymentChange, onNewRequest }: {
  ...
  onHide: (id: string, role: "dealer" | "shop") => void; onFinalPriceChange: (transaction: Transaction, finalPrice: number) => void; onStageChange: (transaction: Transaction, stage: TransactionStage) => void; onPaymentChange: (transaction: Transaction, status: PaymentStatus) => void; onNewRequest: () => void;
}) {
  const [tab, setTab] = useState<"거래내역" | "결제 및 정산">("거래내역");
  const [query, setQuery] = useState(""); const [stageFilter, setStageFilter] = useState<TransactionStage | "전체" | "진행중">("전체"); const [showHidden, setShowHidden] = useState(false);
