...
export function InstallerDetailPanel({
  ...
  const trustRows: { label: string; value: string }[] = [
    ...(installer.reviewCount > 0
      ? [{ label: "평점", value: `★ ${installer.rating.toFixed(1)} (${installer.reviewCount}건 리뷰)` }]
      : []),
    ...(installer.recentTransactionCount > 0
      ? [{ label: "최근 30일 작업", value: `${installer.recentTransactionCount}건` }]
      : []),
  ...
  return (
    <article className="installer-detail-panel">
      ...
      {trustRows.length > 0 && (
        <section className="installer-detail-section installer-detail-section-secondary">
          {trustRows.map((row) => (
            <p key={row.label}>
              <span>{row.label}</span>
              <span>{row.value}</span>
