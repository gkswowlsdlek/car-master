...
export function DealerMapScreen({ query, setQuery, searchArea, location, searchError, results, selectedShop, selectedShopId, setSelectedShopId, favoriteShopIds, toggleFavoriteShop, selectedBrand, isOtherBrand, onRequest }: {
  ...
  return <section className="dealer-screen district-search-screen">
    <div className="page-title"><div><p className="eyebrow">DISTRICT CENTER SEARCH · PATCH 2</p><h1>{location ? `${location.district}청 기준 가까운 시공점` : "행정구역으로 가까운 시공점을 찾으세요"}</h1>{location && <><p>검색 기준: {location.label}</p><small>현재 버전은 행정구역 대표 위치를 기준으로 거리를 계산합니다.</small></>}</div></div>
    <div className="map-search"><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => event.key === "Enter" && void searchArea()} placeholder="예: 강동구, 서울 강동구, 강동구청" /><button className="primary" onClick={() => void searchArea()}>검색</button></div>
