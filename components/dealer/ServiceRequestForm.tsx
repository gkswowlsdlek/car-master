...
const regionDistricts: Record<string, string[]> = {
  서울: ["강남구", "강동구", "서초구", "송파구"],
  경기: ["하남시", "성남시", "구리시", "남양주시"],
  부산: ["해운대구"], 대구: ["수성구"], 인천: ["남동구"], 광주: ["서구"], 대전: ["서구"], 울산: ["남구"], 세종: ["세종시"],
  강원: ["춘천시"], 충북: ["청주시"], 충남: ["천안시"], 전북: ["전주시"], 전남: ["순천시"], 경북: ["포항시"], 경남: ["창원시"], 제주: ["제주시"],
};
...
export function ServiceRequestForm({ request, setRequest, onFindShops, onSummary, hasSelectedShop }: { request: ServiceRequest; setRequest: (request: ServiceRequest) => void; onFindShops: (area?: string) => void; onSummary: () => void; hasSelectedShop: boolean }) {
  ...
  const selectedRegion = Object.keys(regionDistricts).find((region) => request.deliveryArea.includes(region)) ?? "경기";
  const selectedDistrict = regionDistricts[selectedRegion].find((district) => request.deliveryArea.includes(district)) ?? regionDistricts[selectedRegion][0];
  ...
  return <form className="simple-request-card practical-request-card service-request-form">
    ...
    <fieldset className="request-region-field required-field"><legend>시공지역 <em>필수</em></legend><div><select aria-label="시도 선택" value={selectedRegion} onChange={(event) => setArea(event.target.value, regionDistricts[event.target.value][0])}>{Object.keys(regionDistricts).map((region) => <option key={region}>{region}</option>)}</select><select aria-label="시군구 선택" value={selectedDistrict} onChange={(event) => setArea(selectedRegion, event.target.value)}>{regionDistricts[selectedRegion].map((district) => <option key={district}>{district}</option>)}</select></div><small>지역을 선택하면 가까운 시공점 목록과 연결됩니다.</small></fieldset>
