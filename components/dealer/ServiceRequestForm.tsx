...
export function ServiceRequestForm({ request, setRequest, onFindShops, onSummary, hasSelectedShop }: { request: ServiceRequest; setRequest: (request: ServiceRequest) => void; onFindShops: (area?: string) => void; onSummary: () => void; hasSelectedShop: boolean }) {
  ...
  const ready = Boolean(request.model.trim() && request.deliveryArea.trim() && request.workDescription.trim() && request.inboundStart.trim() && request.releaseDate.trim() && hasSelectedShop);
  return <form className="simple-request-card practical-request-card service-request-form">
    ...
    <label className="required-field"><span>출고 희망일 <em>필수</em></span><input required type="date" min={request.inboundStart || today} value={request.releaseDate} onChange={(event) => setRequest({ ...request, releaseDate: event.target.value })} /></label>
