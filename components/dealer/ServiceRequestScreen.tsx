...
export function ServiceRequestScreen({ request, setRequest, shops, selectedShop, selectedShopId, setSelectedShopId, onFindShops, onSummary, onPriceGuide }: {
  request: ServiceRequest; setRequest: (value: ServiceRequest) => void; shops: { shop: InstallerListing; distanceLabel: string }[]; selectedShop: InstallerListing;
  selectedShopId: string; setSelectedShopId: (id: string) => void; onFindShops: (area?: string) => void; onSummary: () => void; onPriceGuide: () => void;
}) {
  const selectedTrust = shopTrustParts(selectedShop, true).join(" · ");
  return <section className="section request-page"><header className="page-title"><div><p className="eyebrow">NEW REQUEST</p><h1>새 시공 요청</h1><p className="page-subtitle">필수 정보만 입력하고 작업을 맡길 시공점을 선택하세요.</p></div></header>
    ...
    <div className="request-layout"><ServiceRequestForm request={request} setRequest={setRequest} onFindShops={onFindShops} onSummary={onSummary} hasSelectedShop={Boolean(selectedShop)} />
      <aside className="card request-shop-panel"><div className="section-heading"><h2>시공점 선택</h2><button className="button button-ghost" onClick={() => onFindShops()}>다시 검색</button></div><div className="request-shop-list">{shops.slice(0, 6).map(({ shop, distanceLabel }) => { const selected = shop.id === selectedShopId; const trust = shopTrustParts(shop, false).join(" · "); return <button key={shop.id} className={selected ? "selected" : ""} onClick={() => setSelectedShopId(shop.id)}><span><b>{shop.name}</b><small>{shop.address}</small></span><em>{distanceLabel}</em>{trust && <i>{trust}</i>}{selected && <strong>선택됨</strong>}</button>; })}</div><div className="selected-installer"><span>선택된 시공점</span><b>{selectedShop.name}</b><small>{selectedShop.address}</small>{selectedTrust && <p>{selectedTrust}</p>}</div></aside>
    </div></section>;
}
