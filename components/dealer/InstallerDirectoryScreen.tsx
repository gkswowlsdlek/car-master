...
export function InstallerDirectoryScreen({
  ...
  const sorted = useMemo(
    () =>
      [...filtered].sort((a, b) => {
        ...
        if (sortKey === "response")
          return responseMinutes(a.installer.responseTime) - responseMinutes(b.installer.responseTime);
  ...
  return (
    <section className="dealer-screen installer-directory">
      ...
      {loading && (
        <p className="installer-loading-note" role="status">
          시공점 정보를 불러오는 중입니다…
        </p>
      )}
        ...
        <div className={`installer-list-pane ${mobileView === "map" ? "mobile-hidden" : ""}`}>
          ...
          {sorted.length === 0 ? (
            <div className="installer-empty-state">
              <span>🔍</span>
              <h2>조건에 맞는 시공점이 없습니다.</h2>
              <p>검색어나 필터를 조정해 보세요.</p>
              <button className="button button-secondary" onClick={resetFilters}>
                필터 초기화
              </button>
              {onShopSearchRequest && (
                <div className="installer-empty-state-shop-search">
                  <p>
                    원하는 시공점을 찾지 못하셨나요?
                    <br />
                    카마스터가 직접 확인해드릴게요.
                  </p>
                  <button className="button button-primary" onClick={onShopSearchRequest}>
                    시공점 찾기 요청
                  </button>
                </div>
              )}
            ...
            <div className="installer-list" ref={listRef}>
              {sorted.map(({ installer, distanceLabel }) => (
                <InstallerCard
                  key={installer.id}
                  installer={installer}
