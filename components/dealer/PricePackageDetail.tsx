...
export function PricePackageDetail({
  item,
  vehicleClass,
  setVehicleClass,
  onQuote,
  ...
  return (
    <aside className="price-package-detail">
      <span>권장 시공 패키지 가이드</span>
      <h2>
        {item.brand} {item.product}
      </h2>
      <div className="vehicle-class-detail-options">
        {vehicleClassOptions.map((option) => (
          <button
            key={option.id}
            className={vehicleClass === option.id ? "active" : ""}
      ...
      <div className="detail-price-box">
        <small>{vehicleClass} 기준</small>
        <b>
          {price.priceRequiresInquiry
      ...
      {vehicleClass === "수입 승용" && (
        <p className="prototype-price-note">수입 승용 5만원 추가는 현재 가이드 계산 기준입니다.</p>
      )}
      <dl>
        <div>
          <dt>브랜드</dt>
        ...
        <div>
          <dt>추가 작업</dt>
          <dd>{item.optionalServices.join(", ")}</dd>
        </div>
        <div>
          <dt>안내</dt>
          <dd>{item.notice}</dd>
