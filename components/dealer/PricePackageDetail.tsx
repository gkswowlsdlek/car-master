...
export function PricePackageDetail({ item, vehicleClass, setVehicleClass, onQuote, onRequest }: { item: PricePackage; vehicleClass: VehicleClass; setVehicleClass: (value: VehicleClass) => void; onQuote: () => void; onRequest: () => void }) {
  ...
  return <aside className="price-package-detail">
    <span>시공 가격 가이드</span><h2>{item.brand} {item.product}</h2>
    ...
    <div className="detail-price-box"><small>{vehicleClass} 기준</small><b>{price.priceRequiresInquiry ? (vehicleClass === "국산 대형/SUV" ? "추가금 발생 가능" : "별도 견적") : formatGuidePrice(price.finalGuidePrice ?? item.guidePrice)}</b></div>
    {vehicleClass === "수입 승용" && <p className="prototype-price-note">수입 승용 5만원 추가는 현재 프로토타입 계산 기준입니다.</p>}
