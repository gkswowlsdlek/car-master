...
export function ShopDashboard({ transactions, onOpenTransactions, onOpenTransaction }: { transactions: Transaction[]; onOpenTransactions: () => void; onOpenTransaction: (id: string) => void }) {
  ...
  const metrics = [
    { label: "신규 요청", value: transactions.filter((item) => item.status.stage === "접수").length, description: "확인과 응답이 필요합니다", tone: "urgent" },
    { label: "오늘 입고", value: transactions.filter((item) => isToday(item.schedule.confirmedInboundAt ?? item.schedule.requestedInboundAt)).length, description: "오늘 예정된 차량입니다", tone: "today" },
    { label: "진행 중", value: transactions.filter((item) => ["입고예정", "입고", "시공중"].includes(item.status.stage)).length, description: "현재 작업 중인 거래입니다", tone: "active" },
    { label: "완료 거래", value: transactions.filter((item) => item.status.stage === "완료").length, description: "누적 완료된 거래입니다", tone: "complete" },
  ];
  ...
  return <section className="shop-dashboard section">
    ...
    <div className="shop-metric-grid">{metrics.map((metric) => <button key={metric.label} className={`shop-metric-card ${metric.tone}`} onClick={onOpenTransactions}><span>{metric.label}</span><b>{metric.value}건</b><small>{metric.description}</small></button>)}</div>
