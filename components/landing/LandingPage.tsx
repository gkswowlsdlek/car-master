...
export function LandingPage({ onStart, onPriceGuide }: { onStart: () => void; onPriceGuide: () => void }) {
  return <main className="marketing-page">
    <nav className="marketing-nav"><div className="marketing-brand"><span className="marketing-brand-logo"><img src="/carmaster-logo-transparent.png" alt="Car-Master" /></span><small>자동차 용품 시공, 더 스마트하게</small></div><div><button className="button button-ghost" onClick={onPriceGuide}>가격 가이드</button><button className="button button-primary" onClick={onStart}>로그인</button></div></nav>
    <section className="marketing-hero">
      <div className="marketing-hero-copy"><div className="hero-kicker"><Sparkles size={15} /> 딜러와 시공점을 위한 업무 플랫폼</div><h1>차량 용품 시공 업무를<br /><span>하나의 거래 흐름으로.</span></h1><p className="page-subtitle hero-message">흩어진 연락과 파일 대신<br />가격 확인부터 시공 완료까지.<br />카마스터가 자동차 용품 업무를<br />선명하게 정리합니다.</p><div className="marketing-cta"><button className="button button-primary button-large" onClick={onStart}>카마스터 시작하기 <ArrowRight size={18} /></button><button className="button button-secondary button-large" onClick={onPriceGuide}>가격 가이드 보기</button></div><div className="hero-assurance"><span><ShieldCheck size={16} /> 거래별 기록 관리</span><span><Clock3 size={16} /> 빠른 업무 확인</span></div></div>
      <div className="hero-product-stack">
        <div className="product-preview" aria-label="카마스터 거래관리 화면 미리보기"><div className="preview-window-bar"><span /><span /><span /><b>Car-Master Workspace</b></div><div className="preview-app"><aside><div className="preview-logo-mark">CM</div>{["대시보드", "가격 가이드", "시공 요청", "거래 관리"].map((item, index) => <i className={index === 0 ? "active" : ""} key={item}>{item}</i>)}</aside><section><header><div><small>TODAY&apos;S WORKSPACE</small><strong>오늘의 시공 업무</strong></div><span>+ 새 시공 요청</span></header><div className="preview-metrics"><article><small>오늘 거래</small><b>12</b><em>+8.2%</em></article><article><small>진행 중</small><b>8</b><em>정상 진행</em></article><article><small>견적 대기</small><b>3</b><em>확인 필요</em></article></div><div className="preview-table"><div><b>최근 거래</b><span>전체 보기</span></div><p><i><Building2 size={14} /></i><span><b>BMW X5</b><small>강동 프리미엄 시공점</small></span><em>시공중</em></p><p><i><Wrench size={14} /></i><span><b>Tesla Model Y</b><small>PPF 패키지 · 오늘 입고</small></span><em className="waiting">입고예정</em></p><p><i><FileText size={14} /></i><span><b>Genesis GV80</b><small>썬팅 패키지 · 견적 확인</small></span><em className="done">완료</em></p></div></section></div></div>
