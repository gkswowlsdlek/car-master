    ...
    <main className="login-ad-page">
      <div className="login-ad-layout">
        ...
        <section className="login-card compact-login-card" ref={loginCardRef}>
          ...
          <p className="login-note">{isDemoAuthMode ? "역할별 베타 아이디로 카마스터 워크스페이스를 시작하세요." : "이메일 계정으로 카마스터 워크스페이스를 시작하세요."}</p>
          <label>{isDemoAuthMode ? "아이디" : "이메일"}<span className="login-input-wrap"><UserRound size={17} /><input ref={emailInputRef} id="login-email" type={isDemoAuthMode ? "text" : "email"} inputMode={isDemoAuthMode ? "numeric" : "email"} autoComplete="username" value={email} onChange={(event) => setEmail(event.target.value)} placeholder={isDemoAuthMode ? "딜러 1 · 시공점 2 · 관리자 3" : "name@company.com"} /></span></label>
