    ...
    <main className="login-ad-page">
      <div className="login-ad-layout">
        ...
        <section className="login-card compact-login-card" ref={loginCardRef}>
          ...
          <p className="login-note">이메일 계정으로 카마스터 워크스페이스를 시작하세요.</p>
          <label>이메일<span className="login-input-wrap"><UserRound size={17} /><input ref={emailInputRef} id="login-email" type="email" autoComplete="username" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@company.com" /></span></label>
          ...
          {isDemoAuthMode && <small className="demo-login-hint">개발용 계정: 딜러 1/1 · 시공점 2/2 · 관리자 3/3</small>}
        </section>
      </div>
    </main>
  );
