...
test("set_transaction_contact_status never touches stage or transaction_stage_events — a phone-contact result is a separate signal, not a work-stage or outcome change", () => {
  const fn = migrationSource.slice(
    migrationSource.indexOf("create function public.set_transaction_contact_status"),
    migrationSource.indexOf("revoke all on function public.set_transaction_contact_status"),
  );
...
test("useTransactionActions exposes setContactStatus, using the real RPC when Supabase-backed and a local-only fallback in Demo (no demo RPC exists for this field)", () => {
  assert.match(
    actionsHookSource,
    /const setContactStatus = useCallback\s*\(\s*async \(transaction: Transaction, status: ContactStatus\)\s*=>\s*\{/,
  );
  assert.match(actionsHookSource, /await supabaseTransactionRepository\.setContactStatus\(transaction\.id, status\)/);
  assert.match(
    actionsHookSource,
    /transactionRepository\.update\s*\(\s*\{\s*\.\.\.transaction,\s*contactStatus:\s*status,/,
  );
});
...
test("TransactionChatWorkspace shows three distinct states for the inline phone-confirm block — unset (record-result buttons), contacted (quiet confirmation, no more nagging), unreachable (retry + find-another-shop, reusing the existing onFindAnotherShop prop)", () => {
  ...
  assert.match(
    chatWorkspaceSource,
    /onClick=\{\(\)\s*=>\s*void recordContactResult\("unreachable"\)\}\s*>\s*연락이 안 돼요/,
  );
  assert.match(
    chatWorkspaceSource,
...
test("TransactionChatWorkspace never auto-changes stage from a contact-result click — recordContactResult only calls onSetContactStatus, nothing stage-related", () => {
  const fn = chatWorkspaceSource.slice(
    chatWorkspaceSource.indexOf("const recordContactResult"),
    chatWorkspaceSource.indexOf("const copyShopMessage"),
  );
...
test("시공점에 보낼 내용 복사 button exists, dealer-only, uses the Clipboard API, and shows a transient '복사됨' state instead of a new toast system", () => {
  assert.match(
    chatWorkspaceSource,
    /role === "dealer"\s*&&\s*(?:\(\s*)?<div className="sidebar-settlement">\s*<h4>시공점에 보낼 내용<\/h4>\s*<button\s+type="button"\s+className="button button-secondary"\s+onClick=\{copyShopMessage\}>/,
  );
...
test("DealerTransactionManagementScreen swaps its 전화 확인 필요 flag to be contact-status-aware — suppressed once contacted, a distinct 연락 안 됨 chip when unreachable", () => {
  ...
  assert.match(
    dealerListSource,
    /contactUnreachable\s*&&\s*(?:\(\s*)?<small className="phone-confirm-flag phone-confirm-flag-unreachable">/,
  );
});
...
test("AdminOpsQueue adds a 연락 실패 거래 card computed locally from the transactions prop (no new query), hidden at zero like every other card", () => {
  ...
  assert.match(
    opsQueueSource,
    /\{\s*key:\s*"unreachable",\s*count:\s*unreachableCount,\s*icon:\s*PhoneOff,\s*label:\s*"연락 실패 거래",\s*onClick:\s*onOpenUnreachableTransactions\s*,?\s*\}/,
  );
});
...
test("AdminTransactionPanel adds a controlled 연락 실패만 filter (same lifted-state pattern as group) and shows 연락 상태 in the detail pane", () => {
  assert.match(
    adminTxnPanelSource,
    /contactFilter:\s*"전체"\s*\|\s*"연락실패";\s*onContactFilterChange:\s*\(value:\s*"전체"\s*\|\s*"연락실패"\)\s*=>\s*void;/,
  );
  assert.match(
    adminTxnPanelSource,
  ...
  assert.match(
    adminTxnPanelSource,
    /<dt>연락 상태<\/dt>\s*<dd>\{selected\.contactStatus\s*\?\s*CONTACT_STATUS_LABEL\[selected\.contactStatus\]\s*:\s*"확인 전"\}<\/dd>/,
  );
});
...
test("a real, Supabase-authenticated Dealer's shop search no longer merges in the ~250-entry fictional demo directory — demoInstallerListings only feeds an actual Demo session", () => {
  assert.match(
    dealerWorkspaceSource,
    /const availableShops = useMemo<InstallerListing\[\]>\s*\(\s*\(\)\s*=>\s*(?:\(\s*)?useSupabaseData\s*\?\s*approvedInstallerShops\s*:\s*demoInstallerListings(?:\s*\))?,\s*\[approvedInstallerShops,\s*useSupabaseData\]\s*,?\s*\);/,
  );
...
test("PrivacyScreen now discloses that Admin can view transaction-room messages for operational purposes, and TermsScreen instructs members not to enter unnecessary third-party personal data in chat", () => {
  ...
  assert.match(
    termsSource,
    /거래와\s+무관한\s+제3자\(예:\s+최종\s+고객\)의\s+이름,\s+연락처,\s+주소\s+등\s+불필요한\s+개인정보는\s+입력하지\s+않아야/,
  );
});
