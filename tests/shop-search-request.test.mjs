...
test("shop_search_requests has exactly the 4 statuses this phase needs — no shop-suggestion/dealer-selection/room fields exist yet", () => {
  assert.match(migrationSource, /create type public\.shop_search_request_status as enum \('requested', 'in_progress', 'cancelled', 'unable_to_connect'\);/);
...
test("create_shop_search_request is dealer-only, snapshots dealer_name/dealer_phone from dealer_profiles at creation time, and validates every required field", () => {
  const fn = migrationSource.slice(migrationSource.indexOf("create or replace function public.create_shop_search_request"), migrationSource.indexOf("revoke all on function public.create_shop_search_request"));
  assert.match(fn, /if caller_role is distinct from 'dealer'::public\.user_role then/);
  assert.match(fn, /select name, phone into dealer_name, dealer_phone\s*\n\s*from public\.dealer_profiles where user_id = auth\.uid\(\);/);
...
test("get_my_shop_search_requests scopes to the caller's own dealer_id and its column list never includes dealer_name/dealer_phone/admin_note", () => {
  const fn = migrationSource.slice(migrationSource.indexOf("create or replace function public.get_my_shop_search_requests"), migrationSource.indexOf("revoke all on function public.get_my_shop_search_requests"));
...
test("cancel_shop_search_request only lets the owning dealer cancel their own open (requested/in_progress) request — never a hard delete, never another dealer's row", () => {
  const fn = migrationSource.slice(migrationSource.indexOf("create or replace function public.cancel_shop_search_request"), migrationSource.indexOf("revoke all on function public.cancel_shop_search_request"));
...
test("admin_start_shop_search_request and admin_mark_shop_search_request_unable are admin-only and only fire from the correct source status", () => {
  const start = migrationSource.slice(migrationSource.indexOf("create or replace function public.admin_start_shop_search_request"), migrationSource.indexOf("revoke all on function public.admin_start_shop_search_request"));
  assert.match(start, /if not public\.is_admin\(\) then raise exception/);
  assert.match(start, /if target\.status <> 'requested' then raise exception 'Only a newly requested item can be started';/);
  assert.match(start, /set status = 'in_progress', updated_at = now\(\)/);

  const unable = migrationSource.slice(migrationSource.indexOf("create or replace function public.admin_mark_shop_search_request_unable"), migrationSource.indexOf("revoke all on function public.admin_mark_shop_search_request_unable"));
...
test("admin_set_shop_search_request_note is admin-only — this is the one write path for the internal-only operational memo", () => {
  const fn = migrationSource.slice(migrationSource.indexOf("create or replace function public.admin_set_shop_search_request_note"), migrationSource.indexOf("revoke all on function public.admin_set_shop_search_request_note"));
  assert.match(fn, /if not public\.is_admin\(\) then raise exception 'Only an administrator can set the operational note';/);
});
...
test("every RPC in this migration keeps the SECURITY DEFINER + empty search_path + revoke-then-grant convention used across the schema", () => {
  for (const name of ["create_shop_search_request", "get_my_shop_search_requests", "cancel_shop_search_request", "get_admin_shop_search_requests", "admin_start_shop_search_request", "admin_mark_shop_search_request_unable", "admin_set_shop_search_request_note"]) {
...
test("types: ShopSearchRequest (dealer view) has no adminNote field; AdminShopSearchRequest extends it with dealer contact + adminNote", () => {
  const dealerType = typesSource.slice(typesSource.indexOf("export type ShopSearchRequest ="), typesSource.indexOf("export type AdminShopSearchRequest"));
...
test("ShopSearchRequestScreen's create form collects every required field (region/workType/vehicle/desiredInboundDate/dateFlexible/optional note) and saves through the repository, never localStorage", () => {
  for (const field of ["region", "workType", "vehicleMaker", "vehicleModel", "desiredInboundDate", "dateFlexible", "dealerNote"]) {
...
test("AdminShopSearchRequestPanel shows Dealer name/phone/region/work/vehicle/desired date/date-flexibility/dealer note, a labeled 'Dealer에게 노출되지 않음' admin-only note field, and gates 확인 시작/연결 어려움 by current status", () => {
  for (const field of ["selected.dealerName", "selected.dealerPhone", "selected.region", "selected.workType", "selected.vehicleMaker", "selected.desiredInboundDate", "selected.dateFlexible", "selected.dealerNote"]) {
