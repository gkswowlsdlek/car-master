...
const migrationUrl = new URL(
  "../supabase/migrations/202608090002_account_foundation_b1_transaction_shop_compatibility.sql",
  import.meta.url,
);
...
test("B1 is one atomic, additive compatibility migration", async () => {
  ...
  assert.match(
    sql,
    /constraint transactions_shop_id_fkey foreign key \(shop_id\) references public\.installer_shops\(id\) on delete restrict/,
  );
...
test("B1 backfills only through the legacy Installer-to-Shop compatibility key", async () => {
  ...
  assert.match(
    sql,
    /update public\.transactions transaction_row set shop_id = shop\.id from public\.installer_shops shop where shop\.legacy_installer_user_id = transaction_row\.installer_id/,
  );
...
test("B1 fails closed before and after backfill", async () => {
  ...
  assert.match(
    sql,
    /where not exists \( select 1 from public\.installer_shops shop where shop\.legacy_installer_user_id = transaction_row\.installer_id \)/,
  );
