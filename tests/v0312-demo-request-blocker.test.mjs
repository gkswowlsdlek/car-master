...
test("the real-only approved-installer alert is strictly inside the useSupabaseData branch, not reachable from Demo", async () => {
  ...
  const createTransactionBody = source.slice(
    createTransactionStart,
    source.indexOf("return <>", createTransactionStart),
  );
  ...
  assert.ok(
    supabaseBranchStart >= 0 && demoBranchStart > supabaseBranchStart,
    "expected useSupabaseData branch to come before useDemoSharedBackend branch",
  );
  assert.ok(
    alertIndex > supabaseBranchStart && alertIndex < demoBranchStart,
    "the approval alert must live strictly inside the useSupabaseData branch, before the Demo branch begins",
  );
});
...
test("demo login explicitly signs out of any leftover real Supabase session before entering Demo", async () => {
  ...
  const authenticateBody = source.slice(
    source.indexOf("const authenticate = useCallback"),
    source.indexOf("const signUp = useCallback"),
  );
  const demoSuccessBranch = authenticateBody.slice(
    authenticateBody.indexOf("if (demoResponse.ok) {"),
    authenticateBody.indexOf("if (demoResponse.status !== 401)"),
  );
  assert.match(
    demoSuccessBranch,
    /authProvider\.logout\(\)/,
    "authenticate()'s demo branch must sign out of any real session before login(demoAccount)",
  );
  ...
  assert.match(
    bootstrapDemoBranch,
    /authProvider\.logout\(\)/,
    "the page-load bootstrap effect's demo branch must also sign out of any real session before login(demoAccount, true)",
  );
});
...
test("migration seeds demo_chat_rooms before demo_transactions (FK-safe on a fresh database)", async () => {
  ...
  const roomsSeedIndex = migration.indexOf(
  ...
  assert.ok(
    roomsSeedIndex < transactionsSeedIndex,
    "demo_chat_rooms seed must run before demo_transactions seed, since chat_room_id is a not-null FK",
  );
});
