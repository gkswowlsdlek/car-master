...
test("no parallel Shop table/type is introduced — the migration only relaxes existing installer_shops NOT NULL columns and adds a traceability column to shop_search_requests", () => {
  ...
  assert.match(
    migrationSource,
    /alter table public\.shop_search_requests add column registered_shop_id uuid references public\.installer_shops\(id\)/,
  );
});
...
test("admin_register_shop is admin-only, creates an approved+unclaimed Shop with no legacy_installer_user_id, no owner, and no forged business paperwork", () => {
  const fn = migrationSource.slice(
    migrationSource.indexOf("create or replace function public.admin_register_shop"),
    migrationSource.indexOf("revoke all on function public.admin_register_shop"),
  );
  assert.match(fn, /if not public\.is_admin\(\) then raise exception 'Only an administrator can register a shop';/);
  assert.match(
    fn,
    /'approved'::public\.installer_approval_status, 'unclaimed'::public\.shop_ownership_status, auth\.uid\(\)/,
  );
...
test("find_duplicate_shop_candidates is admin-only and matches on exact phone OR exact shop_name+address — no fuzzy/similarity matching", () => {
  const fn = migrationSource.slice(
    migrationSource.indexOf("create or replace function public.find_duplicate_shop_candidates"),
    migrationSource.indexOf("revoke all on function public.find_duplicate_shop_candidates"),
  );
...
test("get_admin_shop_search_requests (redefined this phase) still requires is_admin and now also returns registered_shop_id, without dropping any prior column", () => {
  ...
  const fn = migrationSource.slice(
    migrationSource.indexOf("create function public.get_admin_shop_search_requests"),
...
test("AdminShopSearchRequestPanel offers the 시공점 빠른 등록 entry point only while no Shop has been registered yet for that request, and shows a note once one has", () => {
  assert.match(
    panelSource,
    /\{!selected\.registeredShopId\s*&&\s*(?:\(\s*)?<button\s+onClick=\{\(\)\s*=>\s*setRegistrationOpen\(true\)\}>/,
  );
  assert.match(
    panelSource,
    /\{selected\.registeredShopId\s*&&\s*(?:\(\s*)?<p className="admin-quick-shop-registered-note">/,
  );
});
...
test("AdminShopSearchRequestPanel reloads the request list after the modal closes, so 등록됨 state reflects immediately and Admin lands back on the same request", () => {
  assert.match(
    panelSource,
    /<AdminQuickShopRegistrationModal\s+requestId=\{selected\.id\}\s+onClose=\{\(\)\s*=>\s*\{\s*setRegistrationOpen\(false\);\s*void load\(\);\s*\}\}\s+onRegistered=\{\(\)\s*=>\s*void load\(\)\}\s*\/>/,
  );
});
...
test("types: QuickRegisterShopInput's required fields match the product minimum (shopName/address/phone/services/brands), everything else optional", () => {
  const inputType = typesSource.slice(
    typesSource.indexOf("export type QuickRegisterShopInput"),
    typesSource.indexOf("export type DuplicateShopCandidate"),
  );
