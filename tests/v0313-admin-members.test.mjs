...
test("Admin dashboard weekly volume chart is computed from real transaction data, not hardcoded", async () => {
  const source = await read("components/admin/AdminOverview.tsx");
  assert.doesNotMatch(source, /\[38, 52, 44, 68, 58, 82, 72\]/);
  assert.match(source, /function computeWeeklyVolume/);
  assert.match(source, /weeklyVolume\.total === 0/);
});
