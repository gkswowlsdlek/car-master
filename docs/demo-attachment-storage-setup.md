...
이 저장소에는 지금 `SUPABASE_SECRET_KEY`도 `SUPABASE_SERVICE_ROLE_KEY`도
어디에도 설정되어 있지 않다 (`vercel env ls`로 Production/Preview 확인
완료 — `NEXT_PUBLIC_SUPABASE_URL`,
...
room 존재 여부를 자체 검증한 뒤에만 privileged 서버 클라이언트로 업로드/서명
URL 발급을 대행한다.
...
## 2. Vercel에 서버 전용 키 추가

`lib/supabase/service.ts`는 `SUPABASE_SECRET_KEY`를 우선 읽고, 없으면
레거시 이름인 `SUPABASE_SERVICE_ROLE_KEY`로 폴백한다. 둘 다 없으면 기능은
안전하게 비활성 상태(503 / `{ready:false}`)로 유지된다.

1. Supabase 대시보드 → Project Settings → API에서 서버 전용 키를 복사한다
   (Supabase 최신 UI 기준 **Secret key** — 예전 UI에서는 **service_role**
   키로 표기됨. 이 문서에 값을 기록하지 말 것).
2. Vercel 대시보드 → car-master 프로젝트 → Settings → Environment Variables
   → 새 변수 추가:
   - Name: `SUPABASE_SECRET_KEY` (레거시 프로젝트라 Secret key가 아직 없고
     service_role 키만 있다면 `SUPABASE_SERVICE_ROLE_KEY`로 추가해도 동일하게
     동작한다)
   - Value: (복사한 키)
   - Environment: **Production**과 **Preview** 둘 다 체크
   - 절대로 `NEXT_PUBLIC_` 접두사를 붙이지 말 것 — 두 변수 모두 서버 전용이며,
...
`{"ready":false,"reason":"service-role-missing"}`이면 2단계(SUPABASE_SECRET_KEY
또는 SUPABASE_SERVICE_ROLE_KEY 추가)가 안 된 것이고,
...
- 이미지가 즉시 이전 동작(세션 한정)으로 되돌아가야 하면: Vercel에서
  `SUPABASE_SECRET_KEY`(또는 `SUPABASE_SERVICE_ROLE_KEY`)를 삭제하거나
  값을 비우면 된다 — 코드는 다시 자동으로 `LocalAttachmentProvider`로
  폴백한다. 재배포 불필요, 다음 페이지 로드부터 즉시 반영.
