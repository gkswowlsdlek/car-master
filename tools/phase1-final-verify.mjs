...
try {
  ...
  if (disabled) {
    result.issues.push("request-submit-button is disabled after filling all required fields");
  }
  ...
  await page
    .waitForSelector(
      '[data-testid^="transaction-card-"], [data-testid^="deal-card-"], .dealer-active-deals, .deal-list',
      { timeout: 20000 },
    )
    .catch(() => {});
  ...
  await page
    .click('button[aria-label="전화하기"]')
    .catch((e) => result.issues.push("전화하기 button click failed: " + e.message));
  await page.waitForTimeout(1000);
  const contactSheetVisible = await page
    .locator(".contact-sheet-overlay")
    .isVisible()
    .catch(() => false);
  ...
  if (contactSheetVisible) {
    ...
    await page
      .click(".contact-sheet-close")
      .catch((e) => result.issues.push("contact-sheet-close click failed: " + e.message));
    await page
      .waitForSelector(".contact-sheet-overlay", { state: "hidden", timeout: 5000 })
      .catch((e) => result.issues.push("contact-sheet-overlay did not close: " + e.message));
  }

  // 9. Stage: 입고 전 (before) screenshot
  log("STEP 9: screenshot before-status (입고 전)");
  await waitStageText(page, "입고 전", 10000).catch((e) =>
    result.issues.push("stage label 입고 전 not found: " + e.message),
  );
  ...
  await waitStageText(page, "작업 중", 25000).then(
    ...
    (e) => {
      result.steps.stage1to2 = "FAIL";
      result.issues.push("입고 전 -> 작업 중 failed: " + e.message);
    },
  );
  ...
  await page.click('[data-testid="dealer-stage-advance-button"]');
  await page
    .waitForSelector(".stage-confirm-card", { timeout: 8000 })
    .then(async () => {
  ...
  await waitStageText(page, "작업 완료", 25000).then(
    ...
    (e) => {
      result.steps.stage2to3 = "FAIL";
      result.issues.push("작업 중 -> 작업 완료 failed: " + e.message);
    },
  );
  ...
  await waitStageText(page, "출고", 25000).then(
    ...
    (e) => {
      result.steps.stage3to4 = "FAIL";
      result.issues.push("작업 완료 -> 출고 failed: " + e.message);
    },
  );
  ...
  await page
    .waitForSelector("text=DEALER WORKSPACE", { timeout: 20000 })
    .catch((e) => result.issues.push("dashboard not shown after reload: " + e.message));
  if (
    await page
      .locator(".messenger-workspace")
      .isVisible()
      .catch(() => false)
  ) {
    ...
    const dealCard = page
      .locator(
        '[data-testid^="transaction-card-"], [data-testid^="deal-card-"], .deal-list button, .dealer-deal-list button',
      )
      .first();
    if ((await dealCard.count()) > 0) {
      await dealCard.click().catch(() => {});
    }
    await page
      .waitForSelector(".messenger-workspace", { timeout: 15000 })
      .catch((e) => result.issues.push("could not reopen transaction after reload: " + e.message));
  }
  await waitStageText(page, "출고", 15000).then(
    ...
    (e) => {
      result.steps.persistAfterRefresh = "FAIL";
      result.issues.push("stage did not persist as 출고 after refresh: " + e.message);
    },
  );
