...
function publicAccount(role: Role) {
  ...
  return account
    ? {
        ...
        name: account.name,
        role: account.role,
        entryScreen: account.entryScreen,
...
export async function POST(request: NextRequest) {
  ...
  const matched = getDemoCredentials().find(
    (item) => item.username === body?.username?.trim() && item.password === body?.password,
  );
  ...
  try {
    ...
    response.cookies.set(demoSessionCookie, session.token, {
      httpOnly: true,
