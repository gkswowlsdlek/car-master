...
export async function POST(request: NextRequest) {
  if (!isServiceRoleConfigured) return NextResponse.json({ error: "Demo 첨부 기능이 아직 설정되지 않았습니다." }, { status: 503 });
  ...
  const { data: room, error: roomError } = await client.from("demo_chat_rooms").select("id").eq("id", roomId).maybeSingle();
  ...
  const { error: uploadError } = await client.storage.from(DEMO_ATTACHMENTS_BUCKET).upload(storagePath, bytes, { upsert: false, contentType: file.type });
  if (uploadError) return NextResponse.json({ error: "파일 업로드에 실패했습니다." }, { status: 500 });

  const { data: signed, error: signedError } = await client.storage.from(DEMO_ATTACHMENTS_BUCKET).createSignedUrl(storagePath, 3600);
  if (signedError || !signed?.signedUrl) {
    await client.storage.from(DEMO_ATTACHMENTS_BUCKET).remove([storagePath]);
    return NextResponse.json({ error: "첨부파일 접근 주소를 만들지 못했습니다. 잠시 후 다시 시도해 주세요." }, { status: 500 });
  }

  return NextResponse.json({
    id, name: file.name, type: file.type || "application/octet-stream", size: file.size, url: signed.signedUrl, storagePath,
    kind: file.type.startsWith("image/") ? "image" : "file", persistence: "remote", createdAt: new Date().toISOString(),
  });
}
...
export async function DELETE(request: NextRequest) {
  ...
  if (!storagePath || !storagePath.startsWith("demo/")) return NextResponse.json({ error: "잘못된 요청입니다." }, { status: 400 });
