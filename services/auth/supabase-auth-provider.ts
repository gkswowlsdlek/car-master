...
function translateAuthError(error: unknown, fallback: string): string {
  ...
  if (
    ...
    return "비밀번호가 너무 짧거나 취약해요. 다른 비밀번호를 입력해 주세요.";
  if (lower.includes("should be different") || (lower.includes("same") && lower.includes("password")))
  ...
  if (
    ...
    return "링크가 만료되었거나 이미 사용된 링크예요. 다시 요청해 주세요.";
  return fallback;
}
...
export class SupabaseAuthProvider implements AuthProvider {
  ...
  private async resolveUser(user: User): Promise<CurrentUser> {
    ...
    this.currentUser = {
      ...
      name,
      role: profile.role,
      installerId: profile.role === "installer" ? profile.id : undefined,
      approvalStatus,
    };
    return this.currentUser;
  }
    ...
    const metadata =
      input.role === "dealer"
        ? {
            ...
            activity_region: input.activityRegion ?? "",
          }
        : {
            signup_role: "installer",
            ...
            supported_services: input.supportedServices,
            supported_brands: input.supportedBrands,
          };
    const { data, error } = await createSupabaseBrowserClient().auth.signUp({
      email: input.email,
      password: input.password,
      options: { data: metadata, emailRedirectTo: `${origin}/auth/callback?next=/login` },
    });
  ...
  async requestPasswordReset(email: string) {
    ...
    if (error)
      throw new Error(translateAuthError(error, "재설정 이메일을 보내지 못했습니다. 잠시 후 다시 시도해 주세요."));
  }
  ...
  async completeDealerOnboarding(input: DealerOnboardingInput): Promise<CurrentUser> {
    ...
    if (error)
      throw new Error(
        translateOnboardingError(error, "온보딩 정보를 저장하지 못했습니다. 잠시 후 다시 시도해 주세요."),
      );
    return this.resolveUser(user);
  }
  ...
  async completeInstallerOnboarding(input: InstallerOnboardingInput): Promise<CurrentUser> {
    ...
    if (error)
      throw new Error(
        translateOnboardingError(error, "입점 신청 정보를 저장하지 못했습니다. 잠시 후 다시 시도해 주세요."),
      );
    return this.resolveUser(user);
  }
  ...
  async updatePassword(newPassword: string, currentPassword?: string) {
    ...
    if (error)
      throw new Error(translateAuthError(error, "비밀번호를 변경하지 못했습니다. 잠시 후 다시 시도해 주세요."));
  }
  ...
  subscribe(listener: (user: CurrentUser | null) => void) {
    const { data: subscription } = createSupabaseBrowserClient().auth.onAuthStateChange(
      (event: AuthChangeEvent, session: Session | null) => {
        ...
        queueMicrotask(() => {
          void this.resolveUser(session.user)
            .then(listener)
            .catch(() => listener(null));
        });
      },
