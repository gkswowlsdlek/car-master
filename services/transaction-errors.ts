...
export function translateTransactionError(error: unknown, fallback: string): string {
  ...
  if (
    ...
    message.includes("Only the dealer") ||
    message.includes("Only administrators") ||
    message.includes("Only an administrator")
  ) {
    return "이 작업을 처리할 권한이 없습니다.";
  }
  if (message.includes("already ended")) return "이미 종료된 거래입니다.";
  if (message.includes("Invalid final price"))
    return "최종 시공금액을 확인해 주세요. (0보다 크고, 소수점 없는 정수여야 해요)";
  if (message.includes("Invalid transaction outcome") || message.includes("Invalid contact status"))
    return "처리할 수 없는 값입니다. 새로고침 후 다시 시도해 주세요.";
